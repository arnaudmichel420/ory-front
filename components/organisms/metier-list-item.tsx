...
import { BriefcaseBusinessIcon, CpuIcon, LeafIcon } from "lucide-react-native";
...
function MetierListItemCard({ metier }: MetierListItemProps) {
  ...
  const secteurPrincipal =
    ...
    metier.secteurs.find((entry) => entry.secteur?.libelle)?.secteur;
  const secteur = secteurPrincipal?.libelle ?? "Secteur non renseigne";
  const secteurBackground = getMetierSecteurBackground({
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`Voir le metier ${metier.libelle}`}
      onPress={() => {
        router.push(ROUTES.metierDetail(metier.codeOgr));
      }}
    >
        ...
        <CardContent className="gap-4 py-5">
          <View className="flex-row items-start justify-between gap-4">
            ...
            <View className="flex-row items-center gap-2">
              <FlagIcon
                visible={metier.transitionNum ?? false}
                className="rounded-full bg-sky-100 p-2 dark:bg-sky-950/50"
              >
                <CpuIcon size={16} className="text-sky-600 dark:text-sky-300" />
              </FlagIcon>
              <FlagIcon
                visible={metier.transitionEco ?? false}
              ...
              <FlagIcon
                visible={metier.emploiCadre ?? false}
